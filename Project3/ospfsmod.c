...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	int entry_off; //offset from directory block by bytes
	// actual entries
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		if ((entry_off = (f_pos - 2)*OSPFS_DIRENTRY_SIZE) >= dir_oi->oi_size) { // checking if end of directory
			r = 1;
			break;
		}
		...
		switch (entry_oi->oi_ftype) {	//assigning type
		}
		...
		case OSPFS_FTYPE_DIR:
			if (od->od_ino == 0) //ignoring blank entries
				break;
...
allocate_block(void)
{
	...
	for (; bit_map_blockno < ospfs_super->os_firstinob; bit_map_blockno++) { 	//loop through bitmap to look for free space
		void* vector = ospfs_block(bit_map_blockno);
		int i = 0;
		for (; i<OSPFS_BLKBITSIZE; i++) {
			if (bitvector_test(vector, i) == 1) {
				bitvector_clear(vector, i); // if free, allocate and finish
				return (bit_map_blockno - OSPFS_FREEMAP_BLK)*OSPFS_BLKBITSIZE + i;
			}
		}
	}
	return 0;
...
{
	/* EXERCISE: Your code here */
	if (blockno < ospfs_super->os_firstinob) { // out of bounds
		return;
	}
	uint32_t bit_map_blockno = blockno / OSPFS_BLKBITSIZE + OSPFS_FREEMAP_BLK;
	int i = blockno%OSPFS_BLKBITSIZE;
	if (bit_map_blockno >= ospfs_super->os_firstinob) {  //check if block number is valid
		return;
	}
	void* vector = ospfs_block(bit_map_blockno);// if the number was valid, free the block
...
add_block(ospfs_inode_t *oi)
{
	...
	/* EXERCISE: Your code here */
	int32_t indir2Index = indir2_index(n); //index for doubly indirect block
	int32_t indirIndex = indir_index(n); //index for indirect block
	int32_t directIndex = direct_index(n); // index for direct block

	if (indirIndex == -1) {
		oi->oi_direct[directIndex] = allocate_block();
		if (oi->oi_direct[directIndex] == 0){
			return -ENOSPC;
		}
		clear_block(oi->oi_direct[directIndex]);
	}
	else if (indirIndex == 0){	//allocate indirect block
		if (directIndex == 0) {
		...
		uint32_t* indir_block_ptr = (uint32_t*)ospfs_block(oi->oi_indirect);
		indir_block_ptr[directIndex] = allocate_block();
		if (indir_block_ptr[directIndex] == 0) {	// if we cannot allocate block, free indirect block
			if (directIndex == 0) {
		...
		clear_block(indir_block_ptr[directIndex]);
	}
	else if (indir2Index == 0){		//allocate doubly indirect block
		if (indirIndex == 0 && directIndex == 0) {
		...
		if (directIndex == 0) {		//allocate indirect block for doubly indirect block
			double_indir_block_ptr[indirIndex] = allocate_block();
			if (double_indir_block_ptr[indirIndex] == 0) {// if failed, free doubly indirect block
				if (indirIndex == 0) {
			...
			clear_block(double_indir_block_ptr[indirIndex]);
		}
		uint32_t* indir_block_ptr = (uint32_t*)ospfs_block(double_indir_block_ptr[indirIndex]);
		indir_block_ptr[directIndex] = allocate_block();
		if (indir_block_ptr[directIndex] == 0) {	//if direct block cannot be allocated, free indirect block and doubly indirect block
			if (directIndex == 0) {
...
remove_block(ospfs_inode_t *oi)
{
	...
	if (indirIndex == -1) { //just remove entry if no indrect block
		if (oi->oi_direct[directIndex] == 0) 
			return -EIO;
		free_block(oi->oi_direct[directIndex]);
		oi->oi_direct[directIndex] = 0;
	}
	else if (indirIndex == 0){ //free indirect block if it exists
		if (oi->oi_indirect == 0) {
			return -EIO;
		}
		uint32_t* indir_block_ptr = (uint32_t*)ospfs_block(oi->oi_indirect); 
	...
	else if (indir2Index == 0){		//remove doubly indrect block if it exists
		if (oi->oi_indirect2 == 0) {
			return -EIO;
		}
		uint32_t* double_indir_block_ptr = (uint32_t*)ospfs_block(oi->oi_indirect2);
...
change_size(ospfs_inode_t *oi, uint32_t new_size)
{
	...
	while (ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size)) {
	        /* EXERCISE: Your code here */
		r = add_block(oi);
		if (r == -ENOSPC) {	//check if there were no free blocks available and resize the file
			while (ospfs_size2nblocks(oi->oi_size > old_size)) {
				remove_block(oi);	
			}
			return -ENOSPC;
		}
		else if (r == -EIO)	//check for i/o error
			return -EIO; 
	}
	while (ospfs_size2nblocks(oi->oi_size) > ospfs_size2nblocks(new_size)) {	//resizing file
		r = remove_block(oi);
		if (r == -EIO)
			return -EIO; 
	}
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		...
		n = count - amount <= n ? count - amount : n;  //assign n value based on data left in block

		retval = copy_to_user(buffer, data + *f_pos%OSPFS_BLKSIZE, n);
		if (retval != 0) {	//error if unable to write
			retval = -EIO;
			goto done;
		}
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	/* EXERCISE: Your code here */
	if (*f_pos + count>oi->oi_size) {	//check if writing past boundary
		retval = change_size(oi, *f_pos + count); //change the file size
		if (retval != 0) {
			retval = -EIO;
			goto done;
		}
	...
	while (amount < count && retval >= 0) {
		...
		/* EXERCISE: Your code here */
		n = OSPFS_BLKSIZE - *f_pos%OSPFS_BLKSIZE; //check how much data left to write
		n = count - amount <= n ? count - amount : n;
		retval = copy_from_user(data + *f_pos%OSPFS_BLKSIZE, buffer, n);	//copy data, return error on failure
		if (retval != 0) {
			retval = -EIO;
			goto done;
		}
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
	...
	for (; offset < dir_oi->oi_size; offset += OSPFS_DIRENTRY_SIZE) {
		ospfs_direntry_t* entry = (ospfs_direntry_t*)ospfs_inode_data(dir_oi, offset);
		if (entry->od_ino == 0) {	//loop through directory for empty entry
			return entry;
		}
	}
	int retval = add_block(dir_oi);		//adding block if no empty entries
	if (retval != 0) {  
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	...
	if (find_direntry(dir_oi, dst_dentry->d_name.name, dst_dentry->d_name.len) != NULL) {
		return -EEXIST; //check if same file name exists
	}
	
	if (dst_dentry->d_name.len > OSPFS_MAXNAMELEN) {
		return -ENAMETOOLONG; //check if name is too long
	}
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	...
	ospfs_direntry_t* newFileEntry = create_blank_direntry(dir_oi);	//create directory
	...
	for (; current_inode_num<ospfs_super->os_ninodes; current_inode_num++) {	//loop and look for empty inode
	...
	newFileEntry->od_ino = current_inode_num;	//initializing new inode and directory
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	ospfs_direntry_t* symlink_entry = create_blank_direntry(dir); //create new directory
	if (IS_ERR(symlink_entry)) {
		return PTR_ERR(symlink_entry);
	}

	int current_symlink_inode_num = 2;
	ospfs_symlink_inode_t* current_symlink_inode;
	for (; current_symlink_inode_num<ospfs_super->os_ninodes; current_symlink_inode_num++) { //look for empty inode
		current_symlink_inode = ospfs_inode(current_symlink_inode_num);
		if (current_symlink_inode->oi_nlink == 0) { 
			break;
		}
	}
	if (current_symlink_inode_num >= ospfs_super->os_ninodes) {
		return -ENOSPC;
	}

	symlink_entry->od_ino = current_symlink_inode_num;	//initializing new inode
